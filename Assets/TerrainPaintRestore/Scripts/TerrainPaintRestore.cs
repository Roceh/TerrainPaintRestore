...
public class TerrainPaintRestore : MonoBehaviour
{
        ...
        if (terrains != null && terrains.Length > 0)
        {
            ...
            for (int i = 0; i < terrains.Length; i++)
            {
                ...
                for (int n = 0; n < terrains[i].terrainData.detailPrototypes.Length; n++)
                {
                    ...
                    {
                        detailWidth = terrains[i].terrainData.detailWidth,
                        detailHeight = terrains[i].terrainData.detailHeight,
        ...
        if (activeTerrain != null)
        {
            ...
            for (int i = 0; i < terrains.Length; i++)
            {
                ...
                if (restoreTexture)
                {
                    ...
                    if (xx - x > 0 && yy - y > 0)
                    {
                        terrains[i].terrainData.SetAlphamaps(x, y, backup.GetAlphamaps(x, y, xx - x, yy - y));
                    }
                }
                ...
                if (restoreDetails)
                { 
                    for (int n = 0; n < backup.detailLayers.Length; n++)
                    {
                        int x = (int)(rect.x * backup.detailLayers[n].detailWidth);
                        int y = (int)(rect.y * backup.detailLayers[n].detailHeight);
                        int xx = (int)((rect.x + rect.width) * backup.detailLayers[n].detailWidth);
                        int yy = (int)((rect.y + rect.height) * backup.detailLayers[n].detailHeight);

                        x = Math.Min(backup.detailLayers[n].detailWidth, Math.Max(0, x));
                        y = Math.Min(backup.detailLayers[n].detailHeight, Math.Max(0, y));
                        xx = Math.Min(backup.detailLayers[n].detailWidth, Math.Max(0, xx));
                        yy = Math.Min(backup.detailLayers[n].detailHeight, Math.Max(0, yy));

                        if (xx - x > 0 && yy - y > 0)
                        {
                            terrains[i].terrainData.SetDetailLayer(x, y, n, backup.detailLayers[n].GetDetailLayer(x, y, xx - x, yy - y));
                        }
                    }
                }
    ...
    public class GuiDetailLayer
    {
        ...
        public int[,] GetDetailLayer(int x, int y, int width, int height)
        {
            ...
            for (int yy = 0; yy < height; yy++)
            {
                ...
                {
                    if (xx + x >= 0 && xx + x < data.GetLength(1) &&
                        yy + y >= 0 && yy + y < data.GetLength(0))
                    {
                        result[yy, xx] = data[yy + y, xx + x];
                    }
                }
            }
        ...
        public void Load(BinaryReader br)
        {
            ...
            data = new int[detailHeight, detailWidth];
    ...
    public class GuiTerrainData
    {
        ...
        {
            var result = new float[height, width, alphaMaps.GetLength(2)];

            for (int yy = 0; yy < height; yy++)
            {
                for (int xx = 0; xx < width; xx++)
                {
                    for (int zz = 0; zz < alphaMaps.GetLength(2); zz++)
                    {
                        if (yy + y >= 0 && yy + y < alphaMaps.GetLength(0) &&
                            xx + x >= 0 && xx + x < alphaMaps.GetLength(1))
                        {
                            result[yy, xx, zz] = alphaMaps[yy + y, xx + x, zz];
                        }
                    }
                }
            }
